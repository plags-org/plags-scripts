...
exercise_keyとは，システム上の課題のIDであり，ブラウザ上での課題の整列順も規定する文字列である．`build_autograde.py` と `release_as_is.py` では，sourceの拡張子を除いたファイル名がexercise_keyとして利用される．

exercise_keyは正規表現 `'[a-zA-Z0-9_-]{1,64}'` にマッチする文字列でなければならない．したがって，前述のスクリプトを利用する際には，sourceのファイル名もそれに限定される．
...
exercise_keyの実体はipynbのメタデータにあるので，一旦メタデータが設定されたformは自由にリネームして配布できる．ただし，as-isの場合はsourceとformは兼用なので，コピーした上でリネームする方が安全である．
